...
@media print {
	@page {
		margin-top: <?php echo -93 - $purchaseLoop - $returnLoop; ?>%;
		margin-left: -115%;
		margin-right: 0%;
		margin-bottom: 0%;
	}
...
#invoice-POS{
  ...
  width: 120mm;
  background: #FFF;
  height: <?php echo 250 + ($purchaseNum + $returnNum); ?>mm;
...
<hr/>
	...
	<?php
		$sql = "SELECT * FROM qrcodesSales
			...
			ON qrcodesSales.qrCashierId = qrcodesCashier.qrCashierId
			WHERE qrcodesSales.qrSalesTransactNum = '$transactNum' ";
		$resultTransact = $db->prepare($sql);
		$resultTransact->execute();
		for($i=0; $row = $resultTransact->fetch(); $i++){
...
<tr>
	<td colspan = "5"><br/><br/><br/><br/></td>
							...
							<tr class="tabletitle" align = "center">
								<td colspan = "5">Return Item</td>
							</tr>
							<tr class="tabletitle" align = "center">
                                <td><h3>Brand</h3></td>
								<td class="item"><h3>Item</h1></td>
								<td class="Hours"><h3>Qty</h1></td>
								<td class="piece" style = "width: 20%;"><h3>Price</h1></td>
								<td class="Rate"><h3>Sub Total</h1></td>								
							</tr>
<?php
    $sumItemPriceAvail = 0;
    $stringSumItemPriceAvail = "";
	$subTotal = 0;
	$subTotalReturnResult = 0;
    ...
    for($i=0; $row = $resultTransact->fetch(); $i++){
        $brandName = $row['qrSupplierBrandName'];
        $productName = $row['qrSupplierProductName'];
        $qtyAvail = $row['qrReturnItemQtyAvail'];
        $productPrice = number_format( $row['qrProductPrice'], 2, '.', '');
		$subTotal = number_format( ($qtyAvail * $productPrice) , 2, '.', '');
        $salesTaxAmount = $row['qrSalesTaxAmount'];
        $salesAmount = $row['qrSalesAmount'];
		$subTotalReturnResult = number_format( ($subTotalReturnResult + $subTotal), 2, '.', '');
?>
							<tr class="service" align = "center">
								<td class="tableitem"><p class="itemtext"><?php echo $brandName; ?></p></td>
								<td class="tableitem"><p class="itemtext"><?php echo $productName; ?></p></td>
...
<?php
    ...
    if ($subTotalReturnResult == "") {
        $subTotalReturnResult = number_format(0, 2, '.', '');
    }
?>
							<tr class="tabletitle">
							...
							<tr class="tabletitle">
								<td class="payment" colspan = "5" align = "right"><h3 style = "white-space: pre;">Total: <?php echo "Php " . $subTotalReturnResult; ?></h3></td>
							</tr>
						</table>
					</div><!--End Table-->
					<hr class="hr">
